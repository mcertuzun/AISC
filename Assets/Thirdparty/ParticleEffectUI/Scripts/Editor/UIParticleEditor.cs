    ...
    public class UIParticleEditor : GraphicEditor
    {
        class AnimatedPropertiesEditor
        {
            ...
            public static void DrawAnimatableProperties(SerializedProperty sp, Material mat)
            {
                ...
                {
                    var r =
 EditorGUI.PrefixLabel(EditorGUILayout.GetControlRect(true), new GUIContent(sp.displayName, sp.tooltip));
                ...
                if (isClicked)
                {
                    ...
                    for (int i = 0; i < ShaderUtil.GetPropertyCount(mat.shader); i++)
                    {
                        ...
                        {
                            AddMenu(gm, sp, new AnimatedPropertiesEditor() { name = pName + "_ST", type =
 ShaderPropertyType.Vector }, true);
                            AddMenu(gm, sp, new AnimatedPropertiesEditor() { name = pName + "_HDR", type =
 ShaderPropertyType.Vector }, true);
                            AddMenu(gm, sp, new AnimatedPropertiesEditor() { name = pName + "_TexelSize", type =
 ShaderPropertyType.Vector }, true);
                        }
        ...
        //################################
        static readonly GUIContent s_ContentParticleMaterial =
 new GUIContent("Particle Material", "The material for rendering particles");
        static readonly GUIContent s_ContentTrailMaterial =
 new GUIContent("Trail Material", "The material for rendering particle trails");
