...
namespace Util
{
    public class ObjectPool: MonoBehaviour
    {
        ...
        #region SpawnGenericAndGameObjectMethods
        ...
        public static void InternalDeSpawnAfterDelay<T>(T obj, Vector3 position, float delayInSeconds) where T : Component
        {
            InternalDeSpawnAfterDelay(obj.gameObject, position, delayInSeconds);
        }
        
        public static IEnumerator InternalDeSpawnAfterDelay(GameObject obj, Vector3 position, float delayInSeconds)
        {
            Spawn(obj.gameObject, position);
        ...
        {
            GameObject prefab;
            Debug.Log(obj.name + instance.spawnedObjects.TryGetValue(obj, out prefab));
            Debug.Log(instance.spawnedObjects.Count);
            if (instance.spawnedObjects.TryGetValue(obj, out prefab))
                DeSpawn(obj, prefab);
            else
                Object.Destroy(obj);
        }
        static void DeSpawn(GameObject obj, GameObject prefab)
        {
    ...
    public static class ObjectPoolExtensions
    {
        ...
        public static IEnumerator InternalDeSpawnAfterDelay(this GameObject obj, Vector3 position, float delayInSeconds)
        {
            var spawnedObject = ObjectPool.Spawn(obj,null, position, Quaternion.identity);
            yield return new WaitForSeconds(delayInSeconds);
            ObjectPool.DeSpawn(spawnedObject);
        }
        
        public static IEnumerator InternalDeSpawnAfterDelay<T>(this T obj, Vector3 position, float delayInSeconds) where T : Component
        {
            var spawnedObject =ObjectPool.Spawn(obj, null, position, Quaternion.identity);
            yield return new WaitForSeconds(delayInSeconds);
            ObjectPool.DeSpawn(spawnedObject);
        ...
        {
            ObjectPool.DeSpawnAll(prefab);
        }
        public static void DeSpawnAll(this GameObject prefab)
        {
            ObjectPool.DeSpawnAll(prefab);
        }
