...
namespace Champy.AI.Editor
{
    public class BehaviorEditor : EditorWindow
    {
        ...
        {
            editor = GetWindow<BehaviorEditor>();
        ...
        void DrawWindows()
        {
            ...
            settings.currentGraph = (BehaviorGraph) EditorGUILayout.ObjectField(settings.currentGraph,
            ...
            if (clickedOnWindow)
            {
                if (selectedNode.drawNode is StateNode || selectedNode.drawNode is PortalNode)
                {
                    if (selectedNode.id != transitFromId)
                    {
                        ...
                        BaseNode enterNode = settings.currentGraph.GetNodeWithIndex(transNode.enterNode);
                        Transition transition =
                            enterNode.stateRef.currentState.GetTransition(transNode.transRef.transitionId);
        ...
        void ModifyNode(Event e)
        {
            ...
            if (selectedNode.drawNode is StateNode)
            {
                ...
                {
                    menu.AddSeparator("");
                    menu.AddItem(new GUIContent("Add Condition"), false, ContextCallback,
            ...
            switch (a)
            {
                ...
                case UserActions.commentNode:
                    BaseNode commentNode =
                        settings.AddNodeOnGraph(settings.commentNode, 200, 100, "Comment", mousePosition);
